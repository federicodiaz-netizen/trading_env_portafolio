# Este script es un módulo auxiliar que se encarga de leer las listas de tickers
# (acciones, bonos, opciones, etc.) desde la hoja "Tickers" del archivo Excel 
# 'EPGB FE-AB - Python.xlsb'.
#
# Su función principal es crear DataFrames de pandas vacíos, pero con los índices 
# (símbolos de los tickers) correctos. Estos DataFrames son luego utilizados y 
# poblados con datos en tiempo real por el script principal 'main_HB.py'.
#
# Este módulo NO interactúa con el archivo de portafolio ('Portafolio.xlsb'). 
# Esa lógica se encuentra centralizada en 'main_HB.py'.
